...
backend::SamplerType convertSampler(const gli::target gliTarget) {
...
#ifndef NDEBUG
        slog.i << "Filament does not support 1D textures." << io::endl;
#endif
        return backend::SamplerType::SAMPLER_2D;
    case gli::TARGET_1D_ARRAY:
        // Filament没有明确的一维数组纹理采样器类型。
#ifndef NDEBUG
        slog.i << "Filament does not support 1D array textures." << io::endl;
#endif
        return backend::SamplerType::SAMPLER_2D;
    case gli::TARGET_2D:
        return backend::SamplerType::SAMPLER_2D;
...
#ifndef NDEBUG
        slog.i << "Filament does not support rectangle textures." << io::endl;
#endif
        return backend::SamplerType::SAMPLER_2D;
    case gli::TARGET_RECT_ARRAY:
        // Filament没有明确的矩形数组纹理采样器类型。
#ifndef NDEBUG
        slog.i << "Filament does not support rectangle array textures." << io::endl;
#endif
        return backend::SamplerType::SAMPLER_2D;
    ...
    default:
#ifndef NDEBUG
        slog.i << "Unsupported GLI target type for Filament: " + gliTargetString(gliTarget) << io::endl;
#endif
        return backend::SamplerType::SAMPLER_2D;
    }
}
...
inline glm::vec<SIZE_SWIZZLE, backend::TextureSwizzle> convertSwizzles(const gli::swizzles& gliSwizzles) {
    ...
    for (int i = 0; i < SIZE_SWIZZLE; ++i) {
        ...
        default:
#ifndef NDEBUG
            slog.i << "Invalid gli::swizzle value: " + gliSwizzleString(gliSwizzles[i]) << io::endl;
#endif
            swizzles[i] = backend::TextureSwizzle::SUBSTITUTE_ZERO;
        }
    }
...
backend::PixelDataFormat convertPixelDataFormat(const gli::format gliFormat) {
    ...
    default:
#ifndef NDEBUG
        slog.i << "Invalid gli::format value for PixelDataFormat: " + gliFormatString(gliFormat) << io::endl;
#endif
        return PixelDataFormat::UNUSED;
    }
}
...
#ifndef NDEBUG
    slog.i << "Invalid gli::format value for PixelDataType: " + gliFormatString(gliFormat) << io::endl;
#endif
    return PixelDataType::BYTE;
}
...
backend::CompressedPixelDataType convertCompressedPixelDataType(const gli::format gliFormat) {
    ...
    default:
#ifndef NDEBUG
        slog.i << "Invalid gli::format value for CompressedPixelDataType: " + gliFormatString(gliFormat) << io::endl;
#endif
        return CompressedPixelDataType::ETC2_EAC_SRGBA8;
    }
}
...
backend::TextureFormat convertTextureFormat(const gli::format gliFormat) {
    ...
    default:
#ifndef NDEBUG
        slog.i << "Invalid gli::format value for TextureFormat: " + gliFormatString(gliFormat) << io::endl;
#endif
        return TextureFormat::RGBA16F;
    }
}
